...
function Profile() {
  ...
  return (
    <div className="pt-20 px-4 max-w-7xl mx-auto pb-8">
      <div className="glass-effect p-4 sm:p-6 md:p-8 rounded-2xl mb-6 md:mb-8 animate-fade-in">
        <div className="flex flex-col sm:flex-row items-center sm:space-x-6">
          <div className="relative mb-4 sm:mb-0">
            <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-gradient-to-r from-purple-500 to-blue-600 animate-pulse-slow flex items-center justify-center text-2xl font-bold">
              {userInitial}
            </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mb-6 md:mb-8">
        {stats.map((stat, index) => (
          <div
            key={stat.label}
            className="glass-effect p-3 sm:p-4 md:p-6 hover-card animate-fade-in"
            style={{ animationDelay: `${index * 100}ms` }}
          >
            <h3 className={`${isDarkMode ? "text-gray-400" : "text-gray-600"} text-xs sm:text-sm`}>{stat.label}</h3>
            ...
            <div className="flex flex-wrap space-x-2 sm:space-x-4 mb-6">
              {['overview', 'security', 'preferences'].map((tab) => (
                <button
                  ...
                  className={`px-2 sm:px-4 py-2 rounded-lg text-sm sm:text-base transition-all mb-2 sm:mb-0 ${
                    activeTab === tab
                      ? isDarkMode 
                ...
                <div className="space-y-4">
                  <div className="p-3 sm:p-4 bg-white/5 rounded-lg">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <p className={`${isDarkMode ? "text-gray-400" : "text-gray-600"} text-xs sm:text-sm`}>Email</p>
                        <p className="text-sm sm:text-base">{user?.email || 'example@email.com'}</p>
                      </div>
                      <div>
                        <p className={`${isDarkMode ? "text-gray-400" : "text-gray-600"} text-xs sm:text-sm`}>Member Since</p>
                        <p className="text-sm sm:text-base">March 2024</p>
                      </div>
                    </div>
                  </div>
                      ...
                      {activities.map((activity, index) => (
                        <div key={index} className="flex items-center justify-between p-2 hover:bg-white/5 rounded-lg transition-colors">
                          <div className="flex items-center space-x-2 sm:space-x-3">
                            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-r from-purple-500 to-blue-600 flex items-center justify-center">
                          ...
                          <div className="text-right">
                            <p className="font-medium text-sm sm:text-base">{activity.amount}</p>
                            <p className={`text-xs ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>{activity.asset}</p>
                          </div>
                        </div>
                      ))}
                    </div>
          ...
          <Card className="glass-effect sticky top-24">
            ...
            <div className="space-y-3">
              {['Deposit Funds', 'Withdraw', 'API Keys', 'Support'].map((action) => (
                <button
                  key={action}
                  className="w-full p-3 sm:p-4 text-left rounded-lg bg-white/5 hover:bg-white/10 transition-colors flex items-center justify-between text-white text-sm sm:text-base"
                >
                  <span>{action}</span>
